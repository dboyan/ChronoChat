...
public:
  ...
  Contact(const ndn::security::v2::Certificate& certificate,
          bool isIntroducer = false,
          const std::string& alias = "")
    : m_isIntroducer(isIntroducer)
    , m_profile(certificate)
  {
    auto period = certificate.getValidityPeriod().getPeriod();
    m_notBefore = period.first;
    m_notAfter = period.second;

    m_name = m_profile.get("name");
    m_alias = alias.empty() ? m_name : alias;
    m_institution = m_profile.get("institution");

    m_keyName = certificate.getKeyName();
    m_namespace = m_keyName.getPrefix(-1);
    m_publicKey = certificate.getPublicKey();
  }
